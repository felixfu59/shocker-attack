...
2. `file_handle`的`handle_bytes`成员（ext4为8），`handle_type`成员（1）和`f_handle`成员（`inodeid`+4个未知字节）。
...
根据上面的poc可以知道，如果知道`file_handle`的`f_handle`成员（`inodeid`+4个未知字节），即能获取到宿主机的文件。而在宿主机中，根目录 `/`的inodeid一般为2，剩下的4个字节为0。
